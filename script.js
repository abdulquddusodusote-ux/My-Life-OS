/**
 * MENISCUS OS (Simplified, v2) — Application Logic
 * 100% Self-Contained | Zero External Libraries | Zero CDN Failure Points
 * Native Africa/Lagos Timezone Engine | 24-Hour Free-Time Arithmetic
 */
...
const TimeHelper = {
  ...
  getNowParts(timeZone = this.getTimezone()) {
    try {
      ...
      // Calculate Day of Week in target timezone: 0=Sun, 1=Mon, ..., 6=Sat
      const d = new Date(Date.UTC(year, month - 1, day, 12, 0, 0));
      const dayOfWeek = d.getUTCDay();

      return { year, month, day, hour, minute, second, dateKey, timeStr, dayOfWeek, now };
    } catch (e) {
...
const SEED_RHYTHM = [
  { id: 'r1', title: 'High-intensity study block #1', description: 'Deep conceptual focus, problem sets, or heavy medical reading.', targetDuration: '1h 30m' },
...
const App = {
  ...
  selectedCalDateKey: '',
  calYear: 2026,
  calMonthIndex: 8, // 0 = Jan, 8 = Sep
  activeTab: 'home',
  selectedWeekdayTab: 1, // Mon
  ...
  init() {
    try {
      ...
      this.calMonthIndex = nowParts.month - 1; // 0-indexed month
      this.selectedWeekdayTab = nowParts.dayOfWeek;

      this.bindEvents();
      this.startLiveClock();
      this.renderActiveView();
      console.log('MENISCUS OS Initialized Successfully');
    } catch (err) {
      console.error('Initialization error in MENISCUS OS:', err);
    }
  },
  ...
  // ==========================================
  calculateDailyTimeline(dateKey = this.currentDateKey) {
    ...
    dayAnchors.forEach(a => {
      ...
      if (e <= s) return; // Ignore invalid zero/negative duration
    ...
    const totalFreeMinutes = gaps.reduce((acc, g) => acc + g.durationMinutes, 0);

    // Build Chronological Combined Stream of Anchors and Free Windows
    const stream = [];
    let gapIdx = 0;
    let anchorIdx = 0;
    ...
    return {
      ...
      stream,
      totalFreeMinutes,
      totalFreeHoursLabel: this.formatDuration(totalFreeMinutes),
    };
  },
  ...
  renderActiveView() {
    ...
    else if (this.activeTab === 'inbox') this.renderInboxView();
  ...
  updateBadges() {
    ...
    const inboxUnprocessed = inbox.filter(i => !i.done).length;
    const iBadge = document.getElementById('inbox-badge');
    if (iBadge) iBadge.textContent = inboxUnprocessed;
  ...
  renderHomeView() {
    ...
    const timelineData = this.calculateDailyTimeline(this.currentDateKey);

    // Break Day Button State
    ...
    if (breakBtn) {
      if (timelineData.isBreakDay) {
        breakBtn.classList.add('danger');
        breakBtn.classList.remove('secondary');
        if (breakIcon) breakIcon.textContent = '☕';
        if (breakLabel) breakLabel.textContent = 'Break Day Active';
      } else {
        breakBtn.classList.add('secondary');
        breakBtn.classList.remove('danger');
        if (breakIcon) breakIcon.textContent = '☕';
        if (breakLabel) breakLabel.textContent = 'Mark Break Day';
      }
    }
    ...
    if (freeHoursEl) freeHoursEl.textContent = timelineData.totalFreeHoursLabel;
    if (subtextEl) {
      subtextEl.textContent = timelineData.isBreakDay
        ? 'Academic commitments cleared for today. Prayer anchors preserved.'
        : `${timelineData.dayAnchors.length} fixed anchors scheduled. Gaps calculated honestly across 24h.`;
    }

    // Daily Rhythm Circular Progress Ring
    ...
    const allSlotted = AppStore.get(STORAGE_KEYS.SLOTTED_TASKS, {})[this.currentDateKey] || [];
    ...
    if (!timelineData.stream.length) {
      streamContainer.innerHTML = `
        <div class="cushion-card">
          <p class="summary-note">No anchors setup. The full 24 hours (24h) are open.</p>
        </div>`;
      return;
    }
    ...
    timelineData.stream.forEach(item => {
      if (item.type === 'anchor') {
        ...
        el.className = `stream-anchor ${catClass}`;
        el.innerHTML = `
          <div class="anchor-main">
            <span class="anchor-cat-badge">${a.category || 'Anchor'}</span>
            <div>
              <div class="anchor-title">${this.escapeHtml(a.title)}</div>
        ...
        const el = document.createElement('div');
        el.className = 'stream-gap';
        el.innerHTML = `
          <div class="gap-header">
            <div class="gap-duration-pill">
              <span>⏱️ ${g.durationLabel} Free Window</span>
            </div>
            <span class="gap-bounds">${g.startTime} – ${g.endTime}</span>
          </div>
          <div class="slotted-tasks-list" id="gap-tasks-${g.id}">
            ${gapTasks.map(t => `
              <div class="slotted-task ${t.done ? 'done' : ''}">
                ...
                <div class="task-body">
                  <span class="task-tag">${t.source === 'rhythm' ? '⚡ Rhythm' : t.source === 'pool' ? '🎯 Pool' : '✏️ Custom'}</span>
                  <span class="task-title">${this.escapeHtml(t.title)}</span>
                  ${t.notes ? `<div class="task-notes">${this.escapeHtml(t.notes)}</div>` : ''}
                </div>
                <button class="task-remove-btn" data-task-id="${t.id}" title="Remove task">✕</button>
              </div>
            `).join('')}
          </div>
          <div class="gap-action-row">
            <button class="pill-btn secondary small open-slot-modal-btn" data-gap-id="${g.id}" data-gap-label="${g.startTime} – ${g.endTime} (${g.durationLabel})">
  ...
  renderCalendarView() {
    ...
    const firstDay = new Date(this.calYear, this.calMonthIndex, 1).getDay(); // 0=Sun, 1=Mon...
    const firstDayIso = firstDay === 0 ? 7 : firstDay; // 1=Mon... 7=Sun
    ...
    // Trailing days to fill 7-day rows
        ...
        eventsList.innerHTML = dayEvents.map(e => `
          <div class="cal-event-row">
            <div>
              ...
              ${e.time ? `<span class="task-notes font-mono">${e.time}</span>` : ''}
    ...
    container.innerHTML = all.map(e => `
      <div class="item-card">
        <div class="item-content">
          ...
          ${e.time ? `<div class="task-notes font-mono">${e.time}</div>` : ''}
  ...
  renderInboxView() {
    ...
    if (!list.length) {
      container.innerHTML = '<div class="cushion-card"><p class="summary-note">Inbox is clear. Use the field above to jot quick items.</p></div>';
      return;
    }
  ...
  bindEvents() {
    ...
    // Calendar Sub Tabs (Month vs Recurring)
    ...
    document.addEventListener('click', (e) => {
      ...
      if (slotBtn) {
        ...
        const label = slotBtn.dataset.gapLabel;
        this.openSlotModal(gapId, label);
        return;
      }
      ...
      if (taskCheck) {
        ...
        const todayTasks = all[this.currentDateKey] || [];
      ...
      if (removeTaskBtn) {
        ...
        const all = AppStore.get(STORAGE_KEYS.SLOTTED_TASKS, {});
        all[this.currentDateKey] = (all[this.currentDateKey] || []).filter(x => x.id !== taskId);
        AppStore.set(STORAGE_KEYS.SLOTTED_TASKS, all);
        this.renderHomeView();
  ...
  bindModalForms() {
    ...
    sourceSelect?.addEventListener('change', () => {
      ...
      if (v === 'rhythm' && rhythmSelect.value) {
    ...
    slotForm?.addEventListener('submit', (e) => {
      ...
      const all = AppStore.get(STORAGE_KEYS.SLOTTED_TASKS, {});
      if (!all[this.currentDateKey]) all[this.currentDateKey] = [];

      all[this.currentDateKey].push({
        id: `task-${Date.now()}`,
        gapId,
        title,
      ...
      this.renderHomeView();
      this.showToast('Task slotted into window');
    });
    ...
    eventForm?.addEventListener('submit', (e) => {
      ...
      this.renderCalendarView();
      this.showToast('Event saved to calendar');
    });

    // 6. Settings Actions (Timezone, Backup, Reset)
